...
func hit():
	if hitted == false && flying == false:
		...
		$AnimatedSprite2D.offset.y = -3 #set offset, becouse just flipping will make enemy hovers over ground when on backs
